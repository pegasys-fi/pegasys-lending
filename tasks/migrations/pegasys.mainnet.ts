...
task('pegasys:mainnet', 'Deploy development enviroment')
  ...
  .setAction(async ({ verify, skipRegistry }, DRE) => {
    ...
    await DRE.run('full:deploy-address-provider-registry', { pool: POOL_NAME, verify: verify });
    ...
    await DRE.run('full:deploy-lending-pool', { pool: POOL_NAME, verify: verify });

    console.log('3. Deploy oracles');
    await DRE.run('full:deploy-oracles', { pool: POOL_NAME, verify: verify });

    console.log('4. Deploy Data Provider');
    await DRE.run('full:data-provider', { pool: POOL_NAME, verify: verify });

    console.log('5. Deploy WSYS Gateway');
    await DRE.run('full-deploy-wsys-gateway', { pool: POOL_NAME, verify: verify });

    console.log('6. Initialize lending pool');
    await DRE.run('full:initialize-lending-pool', { pool: POOL_NAME, verify: verify });
