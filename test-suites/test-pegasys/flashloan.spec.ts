...
makeSuite('LendingPool FlashLoan function', (testEnv: TestEnv) => {
  ...
  it('Deposits WSYS into the reserve', async () => {
    const { pool, wsys } = testEnv;
    const userAddress = await pool.signer.getAddress();
    const amountToDeposit = ethers.utils.parseEther('1');

    await wsys.mint(amountToDeposit);

    await wsys.approve(pool.address, APPROVAL_AMOUNT_LENDING_POOL);

    await pool.deposit(wsys.address, amountToDeposit, userAddress, '0');
  });

  it('Takes WSYS flashloan with mode = 0, returns the funds correctly', async () => {
    const { pool, helpersContract, wsys } = testEnv;
    ...
    const reserveData = await helpersContract.getReserveData(wsys.address);
  ...
  it('Takes an SYS flashloan with mode = 0 as big as the available liquidity', async () => {
    const { pool, helpersContract, wsys } = testEnv;

    const reserveDataBefore = await helpersContract.getReserveData(wsys.address);
  ...
  it('Takes WSYS flashloan, does not return the funds with mode = 0. (revert expected)', async () => {
    const { pool, wsys, users } = testEnv;
  ...
  it('Takes WSYS flashloan, simulating a receiver as EOA (revert expected)', async () => {
    const { pool, wsys, users } = testEnv;
    const caller = users[1];
    await _mockFlashLoanReceiver.setFailExecutionTransfer(true);
  ...
  it('Takes a WSYS flashloan with an invalid mode. (revert expected)', async () => {
    const { pool, wsys, users } = testEnv;
    const caller = users[1];
  ...
  it('Caller deposits 1000 DAI as collateral, Takes WSYS flashloan with mode = 2, does not return the funds. A variable loan for caller is created', async () => {
    const { dai, pool, wsys, users, helpersContract } = testEnv;
    ...
    const wsysDebtToken = await getVariableDebtToken(variableDebtTokenAddress);

    const callerDebt = await wsysDebtToken.balanceOf(caller.address);
  ...
  it('tries to take a flashloan using a non contract address as receiver (revert expected)', async () => {
    const { pool, deployer, wsys, users } = testEnv;
    const caller = users[1];
  ...
  it('Caller deposits 5 WSYS as collateral, Takes a USDC flashloan with mode = 2, does not return the funds. A loan for caller is created', async () => {
    const { usdc, pool, wsys, users, helpersContract } = testEnv;

    const caller = users[2];

    await wsys.connect(caller.signer).mint(await convertToCurrencyDecimals(wsys.address, '5'));

    await wsys.connect(caller.signer).approve(pool.address, APPROVAL_AMOUNT_LENDING_POOL);

    const amountToDeposit = await convertToCurrencyDecimals(wsys.address, '5');

    await pool.connect(caller.signer).deposit(wsys.address, amountToDeposit, caller.address, '0');
  ...
  it('Caller deposits 1000 DAI as collateral, Takes a WSYS flashloan with mode = 0, does not approve the transfer of the funds', async () => {
    const { dai, pool, wsys, users } = testEnv;
  ...
  it('Caller takes a WSYS flashloan with mode = 1', async () => {
    const { dai, pool, wsys, users, helpersContract } = testEnv;
    ...
    const wsysDebtToken = await getStableDebtToken(stableDebtTokenAddress);
  ...
  it('Caller takes a WSYS flashloan with mode = 1 onBehalfOf user without allowance', async () => {
    const { dai, pool, wsys, users, helpersContract } = testEnv;
  ...
  it('Caller takes a WSYS flashloan with mode = 1 onBehalfOf user with allowance. A loan for onBehalfOf is creatd.', async () => {
    const { dai, pool, wsys, users, helpersContract } = testEnv;
    ...
    const reserveData = await pool.getReserveData(wsys.address);
    ...
    const onBehalfOfDebt = await wsysDebtToken.balanceOf(onBehalfOf.address);
