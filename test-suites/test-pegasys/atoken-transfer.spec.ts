...
const AAVE_REFERRAL = CommonsConfig.ProtocolGlobalParams.PegasysReferral;
...
makeSuite('AToken: Transfer', (testEnv: TestEnv) => {
  ...
  it('User 0 deposits 1000 DAI, transfers to user 1', async () => {
    ...
    expect(name).to.be.equal('Pegasys interest bearing DAI');
  ...
  it('User 0 deposits 1 WSYS and user 1 tries to borrow the WSYS with the received DAI as collateral', async () => {
    const { users, pool, wsys, helpersContract } = testEnv;
    const userAddress = await pool.signer.getAddress();

    await wsys.connect(users[0].signer).mint(await convertToCurrencyDecimals(wsys.address, '1'));

    await wsys.connect(users[0].signer).approve(pool.address, APPROVAL_AMOUNT_LENDING_POOL);

    await pool
      .connect(users[0].signer)
      .deposit(wsys.address, ethers.utils.parseEther('1.0'), userAddress, '0');
    await pool
  ...
  it('User 1 tries to transfer all the DAI used as collateral back to user 0 (revert expected)', async () => {
    const { users, pool, aDai, dai, wsys } = testEnv;
