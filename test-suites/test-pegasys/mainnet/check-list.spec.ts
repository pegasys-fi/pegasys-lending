...
makeSuite('Mainnet Check list', (testEnv: TestEnv) => {
  ...
  it('Deposit WSYS', async () => {
    ...
    await wsysGateway.connect(user.signer).depositSYS(user.address, '0', { value: depositSize });
  ...
  it('Withdraw WSYS - Partial', async () => {
    ...
    const { gasUsed: withdrawGas } = await waitForTx(
      await wsysGateway.connect(user.signer).withdrawSYS(partialWithdraw, user.address)
    );
  ...
  it('Withdraw WSYS - Full', async () => {
    ...
    // Full withdraw
    const { gasUsed: withdrawGas } = await waitForTx(
      await wsysGateway.connect(user.signer).withdrawSYS(MAX_UINT_AMOUNT, user.address)
    );
  ...
  it('Borrow stable WSYS and Full Repay with SYS', async () => {
    const { users, wsysGateway, aWSYS, dai, aDai, wsys, pool, helpersContract } = testEnv;
    ...
    await waitForTx(
      await wsysGateway
        .connect(user.signer)
        .repaySYS(MAX_UINT_AMOUNT, '1', user.address, { value: repaySize })
    );
  ...
  it('Borrow variable WSYS and Full Repay with SYS', async () => {
    ...
    await waitForTx(
      await wsysGateway
        .connect(user.signer)
        .repaySYS(partialPayment, '2', user.address, { value: partialPayment })
    );
    ...
    await waitForTx(
      await wsysGateway
        .connect(user.signer)
        .repaySYS(MAX_UINT_AMOUNT, '2', user.address, { value: repaySize })
    );
  ...
  it('Borrow SYS via delegateApprove SYS and repays back', async () => {
    const { users, wsysGateway, aWSYS, wsys, helpersContract } = testEnv;
    const borrowSize = parseEther('1');
    const user = users[2];
    const { variableDebtTokenAddress } = await helpersContract.getReserveTokensAddresses(
      wsys.address
    ...
    await waitForTx(await wsysGateway.connect(user.signer).borrowSYS(borrowSize, '2', '0'));
    ...
    await waitForTx(
      await wsysGateway
        .connect(user.signer)
        .repaySYS(MAX_UINT_AMOUNT, '2', user.address, { value: borrowSize.mul(2) })
    );
    const debtBalanceAfterFullRepay = await varDebtToken.balanceOf(user.address);
    expect(debtBalanceAfterFullRepay).to.be.eq(zero);
  });
  ...
  it('Getters should retrieve correct state', async () => {
    const { aWSYS, wsys, pool, wsysGateway } = testEnv;

    const WSYSAddress = await wsysGateway.getWSYSAddress();
    const aWSYSAddress = await wsysGateway.getAWSYSAddress();
    const poolAddress = await wsysGateway.getLendingPoolAddress();

    expect(WSYSAddress).to.be.equal(wsys.address);
    expect(aWSYSAddress).to.be.equal(aWSYS.address);
    expect(poolAddress).to.be.equal(pool.address);
  });

  it('Owner can do emergency token recovery', async () => {
    const { users, wsys, dai, wsysGateway, deployer } = testEnv;
    const user = users[0];
    const amount = parseEther('1');

    const uniswapRouter = IUniswapV2Router02Factory.connect(UNISWAP_ROUTER, user.signer);
    await uniswapRouter.swapSYSForExactTokens(
      amount, // 1 DAI
      [wsys.address, dai.address], // Uniswap paths WSYS - DAI
      user.address,
