...
makeSuite('Uniswap adapters', (testEnv: TestEnv) => {
  ...
  const depositAndHFBelowOne = async () => {
    ...
    const amountDAIToBorrow = await convertToCurrencyDecimals(
      dai.address,
      new BigNumber(userGlobalDataBefore.availableBorrowsSYS.toString())
  ...
  const depositSameAssetAndHFBelowOne = async () => {
    ...
    const amountWSYSToBorrow = new BigNumber(userGlobalDataBefore2.availableBorrowsSYS.toString())
      .multipliedBy(0.8)
      .toFixed(0);

    await pool
      .connect(borrower.signer)
      .borrow(wsys.address, amountWSYSToBorrow, RateMode.Variable, '0', borrower.address);
  ...
  describe('Flash Liquidation Adapter', () => {
    ...
    describe('executeOperation: succesfully liquidateCall and swap via Flash Loan with profits', () => {
      it('Liquidates the borrow with profit', async () => {
        ...
        const { dai, wsys, users, pool, oracle, helpersContract, flashLiquidationAdapter } =
          testEnv;
        ...
        const liquidatorWethBalanceBefore = await wsys.balanceOf(liquidator.address);

        // Set how much SYS will be sold and swapped for DAI at Uniswap mock
        await (await mockUniswapRouter.setAmountToSwap(wsys.address, expectedSwap)).wait();
        ...
        const liquidatorWethBalanceAfter = await wsys.balanceOf(liquidator.address);
        ...
        const collateralAssetContractBalance = await wsys.balanceOf(
          flashLiquidationAdapter.address
        );
    ...
    describe('executeOperation: succesfully liquidateCall with same asset via Flash Loan, but no swap needed', () => {
      it('Liquidates the borrow with profit', async () => {
        ...
        const { wsys, users, pool, oracle, helpersContract, flashLiquidationAdapter } = testEnv;
        ...
        const assetPrice = await oracle.getAssetPrice(wsys.address);
        const ethReserveDataBefore = await helpersContract.getReserveData(wsys.address);
        const userReserveDataBefore = await getUserData(
          pool,
          helpersContract,
        ...
        const borrowAssetContractBalance = await wsys.balanceOf(flashLiquidationAdapter.address);
    ...
    describe('executeOperation: invalid params', async () => {
      it('Revert if debt asset is different than requested flash loan token', async () => {
        ...
        const { dai, wsys, users, pool, helpersContract, flashLiquidationAdapter } = testEnv;
        ...
        const params = buildFlashLiquidationAdapterParams(
          wsys.address,
          wsys.address, // intentionally bad
          borrower.address,
          amountToLiquidate,
          false
        );
