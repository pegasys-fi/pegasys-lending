...
makeSuite('LendingPool liquidation - liquidator receiving the underlying asset', (testEnv) => {
  ...
  it("It's not possible to liquidate on a non-active collateral or a non active principal", async () => {
    const { configurator, wsys, pool, users, dai } = testEnv;
    const user = users[1];
    await configurator.deactivateReserve(wsys.address);

    await expect(
      pool.liquidationCall(wsys.address, dai.address, user.address, parseEther('1000'), false)
  ...
  it('Deposits WSYS, borrows DAI', async () => {
    const { dai, wsys, users, pool, oracle } = testEnv;
    ...
    //user 2 deposits 1 SYS
    const amountSYStoDeposit = await convertToCurrencyDecimals(wsys.address, '1');

    //mints WSYS to borrower
    await wsys.connect(borrower.signer).mint(await convertToCurrencyDecimals(wsys.address, '1000'));
    ...
    const amountDAIToBorrow = await convertToCurrencyDecimals(
      dai.address,
      new BigNumber(userGlobalData.availableBorrowsSYS.toString())
  ...
  it('Liquidates the borrow', async () => {
    const { dai, wsys, users, pool, oracle, helpersContract } = testEnv;
    ...
    const ethReserveDataBefore = await helpersContract.getReserveData(wsys.address);
    ...
    const tx = await pool
      ...
      .liquidationCall(wsys.address, dai.address, borrower.address, amountToLiquidate, false);
    ...
    const ethReserveDataAfter = await helpersContract.getReserveData(wsys.address);

    const collateralPrice = await oracle.getAssetPrice(wsys.address);
  ...
  it('User 3 deposits 1000 USDC, user 4 1 WSYS, user 4 borrows - drops HF, liquidates the borrow', async () => {
    const { usdc, users, pool, oracle, wsys, helpersContract } = testEnv;
    ...
    await pool
      .connect(liquidator.signer)
      .liquidationCall(wsys.address, usdc.address, borrower.address, amountToLiquidate, false);
  ...
  it('User 4 deposits 10 AAVE - drops HF, liquidates the AAVE, which results on a lower amount being liquidated', async () => {
    const { pegasys, usdc, users, pool, oracle, helpersContract } = testEnv;
    ...
    await pegasys
      .connect(borrower.signer)
      .mint(await convertToCurrencyDecimals(pegasys.address, '10'));

    //approve protocol to access the borrower wallet
    await pegasys.connect(borrower.signer).approve(pool.address, APPROVAL_AMOUNT_LENDING_POOL);

    //borrower deposits 10 AAVE
    const amountToDeposit = await convertToCurrencyDecimals(pegasys.address, '10');

    await pool
      .connect(borrower.signer)
      .deposit(pegasys.address, amountToDeposit, borrower.address, '0');
    const usdcPrice = await oracle.getAssetPrice(usdc.address);
    ...
    const usdcReserveDataBefore = await helpersContract.getReserveData(usdc.address);
    const pegasysReserveDataBefore = await helpersContract.getReserveData(pegasys.address);
    ...
    const collateralPrice = await oracle.getAssetPrice(pegasys.address);
    const principalPrice = await oracle.getAssetPrice(usdc.address);

    await pool
      .connect(liquidator.signer)
      .liquidationCall(pegasys.address, usdc.address, borrower.address, amountToLiquidate, false);
    ...
    const usdcReserveDataAfter = await helpersContract.getReserveData(usdc.address);
    const pegasysReserveDataAfter = await helpersContract.getReserveData(pegasys.address);

    const pegasysConfiguration = await helpersContract.getReserveConfigurationData(pegasys.address);
    const collateralDecimals = pegasysConfiguration.decimals.toString();
    const liquidationBonus = pegasysConfiguration.liquidationBonus.toString();
    ...
    expect(pegasysReserveDataAfter.availableLiquidity.toString()).to.be.bignumber.almostEqual(
      new BigNumber(pegasysReserveDataBefore.availableLiquidity.toString())
        .minus(expectedCollateralLiquidated)
        .toFixed(0),
      'Invalid collateral available liquidity'
    );
