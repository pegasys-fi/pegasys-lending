...
makeSuite('Pausable Pool', (testEnv: TestEnv) => {
  ...
  it('Flash loan', async () => {
    const { dai, pool, wsys, users, configurator } = testEnv;
  ...
  it('Liquidation call', async () => {
    const { users, pool, usdc, oracle, wsys, configurator, helpersContract } = testEnv;
    ...
    // Do liquidation
    await expect(
      pool.liquidationCall(wsys.address, usdc.address, borrower.address, amountToLiquidate, true)
    ).revertedWith(LP_IS_PAUSED);
  ...
  it('SwapBorrowRateMode', async () => {
    const { pool, wsys, dai, usdc, users, configurator } = testEnv;
    const user = users[1];
    const amountWSYSToDeposit = parseEther('10');
    const amountDAIToDeposit = parseEther('120');
    const amountToBorrow = parseUnits('65', 6);

    await wsys.connect(user.signer).mint(amountWSYSToDeposit);
    await wsys.connect(user.signer).approve(pool.address, APPROVAL_AMOUNT_LENDING_POOL);
    await pool.connect(user.signer).deposit(wsys.address, amountWSYSToDeposit, user.address, '0');
  ...
  it('setUserUseReserveAsCollateral', async () => {
    ...
    const amountWSYSToDeposit = parseEther('1');
    await wsys.connect(user.signer).mint(amountWSYSToDeposit);
    await wsys.connect(user.signer).approve(pool.address, APPROVAL_AMOUNT_LENDING_POOL);
    await pool.connect(user.signer).deposit(wsys.address, amountWSYSToDeposit, user.address, '0');
    ...
    await expect(
      pool.connect(user.signer).setUserUseReserveAsCollateral(wsys.address, false)
    ).revertedWith(LP_IS_PAUSED);
