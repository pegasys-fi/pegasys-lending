...
makeSuite('LendingPool liquidation - liquidator receiving aToken', (testEnv) => {
  ...
  it('Deposits WSYS, borrows DAI/Check liquidation fails because health factor is above 1', async () => {
    ...
    await expect(
      pool.liquidationCall(wsys.address, dai.address, borrower.address, 1, true)
  ...
  it('Liquidates the borrow', async () => {
    const { pool, dai, wsys, aWSYS, aDai, users, oracle, helpersContract, deployer } = testEnv;
    const borrower = users[1];
  ...
  it('User 3 deposits 1000 USDC, user 4 1 WSYS, user 4 borrows - drops HF, liquidates the borrow', async () => {
    const { users, pool, usdc, oracle, wsys, helpersContract } = testEnv;
