...
import { MockParaSwapAugustus } from '../../typechain/MockParaSwapAugustus';
import { MockParaSwapAugustusRegistry } from '../../typechain/MockParaSwapAugustusRegistry';
  ...
  describe('ParaSwapLiquiditySwapAdapter', () => {
    ...
    describe('executeOperation', () => {
      beforeEach(async () => {
        ...
        await wsys.mint(parseEther('10000'));
        await wsys.approve(pool.address, parseEther('10000'));
        await pool.deposit(wsys.address, parseEther('10000'), deployer.address, 0);
      ...
      it('should correctly swap tokens and deposit the out tokens in the pool', async () => {
        const { users, wsys, oracle, dai, aDai, aWSYS, pool, paraswapLiquiditySwapAdapter } =
          testEnv;
        ...
        const flashloanPremium = amountWSYStoSwap.mul(9).div(10000);
        const flashloanTotal = amountWSYStoSwap.add(flashloanPremium);

        // User will swap liquidity aEth to aDai
        const userAEthBalanceBefore = await aWSYS.balanceOf(userAddress);
        await aWSYS.connect(user).approve(paraswapLiquiditySwapAdapter.address, flashloanTotal);
        ...
        await expect(
          ...
          .withArgs(wsys.address, dai.address, amountWSYStoSwap, expectedDaiAmount);

        const adapterWethBalance = await wsys.balanceOf(paraswapLiquiditySwapAdapter.address);
        ...
        expect(userAEthBalance).to.be.lte(userAEthBalanceBefore.sub(amountWSYStoSwap));
      });
      ...
      it('should revert if caller not lending pool', async () => {
        const { users, wsys, oracle, dai, aWSYS, paraswapLiquiditySwapAdapter } = testEnv;
        const user = users[0].signer;
        const userAddress = users[0].address;
        ...
        await expect(
          paraswapLiquiditySwapAdapter
            .connect(user)
            .executeOperation([wsys.address], [amountWSYStoSwap], [0], userAddress, params)
      ...
      it('should revert when min amount to receive exceeds the max slippage amount', async () => {
        const { users, wsys, oracle, dai, aWSYS, pool, paraswapLiquiditySwapAdapter } = testEnv;
        const user = users[0].signer;
        const userAddress = users[0].address;
      ...
      it('should correctly swap tokens all the balance', async () => {
        ...
        // User will swap liquidity aEth to aDai
        const userAEthBalanceBefore = await aWSYS.balanceOf(userAddress);
        expect(userAEthBalanceBefore).to.be.eq(amountWSYStoSwap.add(flashloanPremium));
      ...
      it('should revert trying to swap more than balance', async () => {
        ...
        const amountWSYStoSwap = await convertToCurrencyDecimals(wsys.address, '101');
      ...
      it('should not touch any token balance already in the adapter', async () => {
        ...
        await wsys.mint(adapterWethBalanceBefore);
        await wsys.transfer(paraswapLiquiditySwapAdapter.address, adapterWethBalanceBefore);
    ...
    describe('executeOperation with borrowing', () => {
      beforeEach(async () => {
        ...
        await pool
          .connect(borrower)
          .borrow(wsys.address, parseEther('5000'), 2, 0, borrowerAddress);
      });
      ...
      it('should correctly swap tokens and deposit the out tokens in the pool', async () => {
        ...
        expect(userAEthBalance).to.be.lt(
          userAEthBalanceBefore.mul(10001).div(10000).sub(amountWSYStoSwap)
        );
      });
      ...
      it('should correctly swap tokens all the balance', async () => {
        ...
        await mockAugustus.expectSwap(
          wsys.address,
          dai.address,
          amountWSYStoSwap.add(1),
          amountWSYStoSwap.mul(10001).div(10000),
          expectedDaiAmount
        );
    ...
    describe('swapAndDeposit', () => {
      ...
      it('should correctly swap tokens and deposit the out tokens in the pool', async () => {
        const { users, wsys, oracle, dai, aDai, aWSYS, paraswapLiquiditySwapAdapter } = testEnv;
        const user = users[0].signer;
        const userAddress = users[0].address;
        ...
        // User will swap liquidity aEth to aDai
        const userAEthBalanceBefore = await aWSYS.balanceOf(userAddress);
        await aWSYS.connect(user).approve(paraswapLiquiditySwapAdapter.address, amountWSYStoSwap);
        ...
        expect(adapterDaiBalance).to.be.eq(Zero);
        expect(userADaiBalance).to.be.eq(expectedDaiAmount);
        expect(userAEthBalance).to.be.eq(userAEthBalanceBefore.sub(amountWSYStoSwap));
      });
      ...
      it('should revert when trying to swap more than balance', async () => {
        ...
        const amountWSYStoSwap = (await convertToCurrencyDecimals(wsys.address, '100')).add(1);
      ...
      it('should revert when trying to swap more than allowance', async () => {
        ...
        await aWSYS
          .connect(user)
          .approve(paraswapLiquiditySwapAdapter.address, amountWSYStoSwap.sub(1));
      ...
      it("should revert if Augustus doesn't swap correct amount", async () => {
        ...
        const augustusSwapAmount = amountWSYStoSwap.sub(1);
      ...
      it('should correctly swap all the balance when using a bigger amount', async () => {
        ...
        // User will swap liquidity aEth to aDai
        const userAEthBalanceBefore = await aWSYS.balanceOf(userAddress);
        expect(userAEthBalanceBefore).to.be.eq(amountWSYStoSwap);

        const bigAmountToSwap = parseEther('11');
        await aWSYS.connect(user).approve(paraswapLiquiditySwapAdapter.address, bigAmountToSwap);
      ...
      it('should revert trying to swap all the balance when using a smaller amount', async () => {
        ...
        await aWSYS.connect(user).approve(paraswapLiquiditySwapAdapter.address, smallAmountToSwap);
    ...
    describe('swapAndDeposit with borrowing', () => {
      ...
      it('should correctly swap tokens and deposit the out tokens in the pool', async () => {
        ...
        expect(adapterDaiBalance).to.be.eq(Zero);
        expect(userADaiBalance).to.be.eq(expectedDaiAmount);
        expect(userAEthBalance).to.be.gt(userAEthBalanceBefore.sub(amountWSYStoSwap));
        expect(userAEthBalance).to.be.lt(
          userAEthBalanceBefore.mul(10001).div(10000).sub(amountWSYStoSwap)
        );
      });
